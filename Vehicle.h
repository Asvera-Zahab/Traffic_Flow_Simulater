...
public:
    ...
    Vehicle() {
        ...
        remainingTravelTime = 0.0; pathIndex = 0; status = WAITING;
        stepEntered = 0; stepArrived = -1; totalDelay = 0.0;
    }

    Vehicle(int vid, int src, int dst, int step) {
        id = vid; source = src; destination = dst; currentNode = src; currentRoad = -1; remainingTravelTime = 0.0;
    ...
    void display() const {
        cout << "  Vehicle " << id << " | " << source << "->" << destination << " | At node: " << currentNode
            << " | Status: " << getStatusString() << " | RemainingTime: " << remainingTravelTime << endl;
    }
};